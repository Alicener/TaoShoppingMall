...
public class CalendarView extends View implements View.OnTouchListener {
	...
	private OnMoveLeftListener  onMoveLeftListener ;
...
//		canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);
	...
	private void drawCellBg(Canvas canvas, int index, int color) {
		int x = getXByIndex(index);
		int y = getYByIndex(index);
		surface.cellBgPaint.setColor(Color.RED);
		...
		float cx = (right-left)/2 + left;
		float cy = (bottom - top)/2 +top + 10;
		float radius = 40;
...
//		}
	...
	public boolean onTouch(View v, MotionEvent event) {
        ...
        if(event.getAction() == MotionEvent.ACTION_UP) {  
            ...
            y2 = event.getY();  
            if(y1 - y2 > 50) {  
                //Toast.makeText(context, "向上滑", Toast.LENGTH_SHORT).show();  
            } else if(y2 - y1 > 50) {  
                //Toast.makeText(context, "向下滑", Toast.LENGTH_SHORT).show();  
            } else if(x1 - x2 > 50) {  
                //Toast.makeText(context, "向左滑", Toast.LENGTH_SHORT).show(); 
            	onMoveLeftListener.onMoveLeft();
            } else if(x2 - x1 > 50) {  
...
//		}
	...
	public void setOnMoveLeftListener (OnMoveLeftListener  moveleft){
		this.onMoveLeftListener  = moveleft;
	}
	...
	private class Surface {
		...
		public void init() {
			...
			cellHeight = (height - monthHeight - weekHeight) / 6f + 7;
			cellWidth = width / 7f;
